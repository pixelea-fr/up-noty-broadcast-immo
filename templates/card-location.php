...
?>
<div class="up-immo-card up-immo-card--location">
    ...
    <?php if ( $charges_incluses !== '' || $montant_charges !== '' ) : ?>
        <p class="up-immo-card__charges">
            <?php
            $txt = array();
            if ( $charges_incluses === '1' ) {
                $txt[] = 'Charges incluses';
            } elseif ( $charges_incluses === '0' ) {
                $txt[] = 'Charges non incluses';
            }
            if ( $montant_charges !== '' ) {
                $txt[] = 'Charges: ' . number_format( (float) $montant_charges, 0, ',', ' ' ) . ' €';
            }
            echo esc_html( implode( ' · ', $txt ) );
            ?>
        </p>
    <?php endif; ?>
</div>
